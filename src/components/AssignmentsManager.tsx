...
export interface AssignmentsManagerProps { role: "admin" | "instructor"; sidebarItems: any; }
...
const AssignmentsManager = ({ role, sidebarItems }: AssignmentsManagerProps) => {
  ...
  const fetchAssignments = async (overrideBatches?: Batch[]) => {
    try {
      // Use the new instructor-assignments endpoint for filtering
      const params: any = {};
      if (filterCourse) params.course_id = filterCourse;
      if (filterBatch) params.batch_id = filterBatch;
      if (filterStatus) params.status = filterStatus;
      if (role === "admin" && filterInstructor) params.instructor_id = filterInstructor;

      const res = await instructorService.getInstructorAssignments(params);
      const data: AssignmentApiItem[] = Array.isArray(res) ? res : (res?.results || []);

      const currentBatches = overrideBatches || batches;

      const mapped = data.map((item) => {
        // Handle both flat and nested structures for assignments and related entities
        const assignment = item.assignment || item;
        const bId = assignment.batch || item.batch;
        const batchInfo = currentBatches.find(b => b.id === bId);

        // Resiliently resolve IDs and titles
        const courseId = item.course?.id || (typeof item.course === 'number' ? item.course : batchInfo?.course_id) || 0;
        const courseTitle = item.course?.title || batchInfo?.course_title || (typeof item.course === 'object' ? item.course?.title : "Course");
        const moduleId = item.module?.id || (typeof item.module === 'number' ? item.module : 0);
        const moduleTitle = item.module?.title || "Module";
        const lessonId = item.lesson?.id || (typeof item.lesson === 'number' ? item.lesson : 0);
        const lessonTitle = item.lesson?.title || assignment.assignment_title || "Lesson";
        ...
        return {
          ...
          assignmentTitle: assignment.assignment_title || lessonTitle || "Untitled",
          assignmentDescription: assignment.assignment_description || "",
          assignmentDueDate: assignment.assignment_due_date || null,
  ...
  useEffect(() => {
    const boot = async () => {
      try {
        ...
        // Fetch batches first so they are available for assignment mapping
        const batchesData = await (role === "admin" ? batchService.getAllBatches() : batchService.getInstructorBatches());
        const bList = Array.isArray(batchesData) ? batchesData : (batchesData?.results || []);
        setBatches(bList);

        const promises: Promise<any>[] = [
          fetchAssignments(bList),
        ];

        if (role === "admin") {
          promises.push(instructorService.getAllInstructors());
        }

        const results = await Promise.all(promises);

        if (role === "admin") {
          const instData = results[1];
          setInstructorsList(Array.isArray(instData) ? instData : (instData?.results || []));
        }
      } catch (err) {
        console.error("Failed to boot assignments manager", err);
      } finally {
  ...
  }, [filterCourse, filterBatch, filterStatus, filterInstructor]);

  const courses = useMemo(() => {
    const map = new Map<number, string>();
    batches.forEach((b) => {
      // Handle both flat and nested course structures in batches
      const cId = b.course_id || (typeof b.course === 'object' ? b.course?.id : b.course);
      const cTitle = b.course_title || (typeof b.course === 'object' ? b.course?.title : "Course");
      if (cId) map.set(Number(cId), cTitle);
    });
  ...
  const filteredBatchesForForm = useMemo(() => {
    ...
    return batches.filter((b) => {
      const cId = b.course_id || (typeof b.course === 'object' ? b.course?.id : b.course);
      return String(cId) === formData.courseId;
    });
  }, [batches, formData.courseId]);

  // Dynamic filter options based on available assignments and current selection
  const filterOptions = useMemo(() => {
    ...
    // We use all assignments to populate the top-level filters, 
    // but we can also use the master lists (instructorsList, batches) for better coverage.

    // For Courses: Filter by selected instructor
    batches.forEach(b => {
      const instructorId = b.instructor;
      const courseId = b.course_id || (typeof b.course === 'object' ? b.course?.id : b.course);
      const courseTitle = b.course_title || (typeof b.course === 'object' ? b.course?.title : "Course");

      if (!filterInstructor || String(instructorId) === filterInstructor) {
        if (courseId) coursesMap.set(Number(courseId), courseTitle);
      }
    });

    // For Batches: Filter by selected instructor and selected course
    batches.forEach(b => {
      const instructorId = b.instructor;
      const courseId = b.course_id || (typeof b.course === 'object' ? b.course?.id : b.course);

      const matchInstructor = !filterInstructor || String(instructorId) === filterInstructor;
      const matchCourse = !filterCourse || String(courseId) === filterCourse;

      if (matchInstructor && matchCourse) {
        batchesMap.set(b.id, b.name);
      }
    });

    return {
      courses: Array.from(coursesMap.entries()).map(([id, title]) => ({ id, title })),
      batches: Array.from(batchesMap.entries()).map(([id, name]) => ({ id, name })),
    };
  }, [batches, filterInstructor, filterCourse, role]);
  ...
  const filteredAssignments = useMemo(() => {
    return assignments.filter((a) => {
      // Security filter: Instructors should only see assignments for batches they handle
      if (role === "instructor") {
        const hasAccess = batches.some(b => b.id === a.batch);
        if (!hasAccess) return false;
      }

      const matchCourse = filterCourse ? String(a.courseId) === filterCourse : true;
      const matchStatus = filterStatus ? a.status === filterStatus : true;
      const matchBatch = filterBatch ? String(a.batch) === filterBatch : true;
      const matchSearch = a.assignmentTitle.toLowerCase().includes(filterSearch.toLowerCase());
      return matchCourse && matchStatus && matchBatch && matchSearch;
    });
  }, [assignments, filterCourse, filterStatus, filterBatch, filterSearch, role, batches]);
  ...
  if (initialLoading) {
    return (
      <DashboardLayout role={role} sidebarItems={sidebarItems} title="Assignments">
    ...
    <DashboardLayout role={role} sidebarItems={sidebarItems} title="Assignments">
      ...
      <Card className="mb-6 shadow-sm">
        <CardContent className="p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
          ...
          {role === "admin" && (
            <div>
              <Label className="text-xs text-gray-500 mb-1 block">Filter Instructor</Label>
              <select className="flex h-10 w-full rounded-md border border-input bg-transparent px-3 py-2 text-sm shadow-sm transition-colors" value={filterInstructor} onChange={(e) => {
                setFilterInstructor(e.target.value);
                setFilterCourse("");
                setFilterBatch("");
              }}>
                <option value="">All Instructors</option>
                {instructorsList.map((inst) => <option key={inst.id} value={inst.username || String(inst.id)}>{inst.username || inst.full_name || inst.name || inst.email}</option>)}
              </select>
            </div>
          )}
          <div>
            <Label className="text-xs text-gray-500 mb-1 block">Filter Course</Label>
            <select className="flex h-10 w-full rounded-md border border-input bg-transparent px-3 py-2 text-sm shadow-sm transition-colors" value={filterCourse} onChange={(e) => { setFilterCourse(e.target.value); setFilterBatch(""); }}>
              <option value="">All Courses</option>
              {filterOptions.courses.map((course) => <option key={course.id} value={String(course.id)}>{course.title}</option>)}
            </select>
          </div>
          <div>
            ...
            <select className="flex h-10 w-full rounded-md border border-input bg-transparent px-3 py-2 text-sm shadow-sm transition-colors" value={filterBatch} onChange={(e) => setFilterBatch(e.target.value)}>
              ...
              {filterOptions.batches.map((b) => <option key={b.id} value={String(b.id)}>{b.name}</option>)}
            </select>
          </div>
          <div>
          ...
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 border-b text-slate-500 font-medium">
              <tr>
                ...
                {role === "admin" && <th className="px-4 py-3">Instructor</th>}
            ...
            <tbody className="divide-y text-gray-700">
              {filteredAssignments.length === 0 ? (
                <tr>
                  <td colSpan={role === "admin" ? 7 : 6} className="py-8 text-center text-gray-500">No assignments found matching filters.</td>
                </tr>
              ) : (
                    ...
                    <tr key={`${assignment.moduleId}-${assignment.lessonId}`} className="hover:bg-slate-50 transition-colors">
                      ...
                      {role === "admin" && (
                        <td className="px-4 py-3">
                          <span className="text-sm font-medium text-gray-700">{assignment.instructorName}</span>
                        </td>
                      )}
                      <td className="px-4 py-3">
                        <div className="flex flex-col">
                          <span className="font-medium text-[#000080]">{assignment.courseTitle}</span>
                          <span className="text-xs text-gray-500">{assignment.batchName} • {assignment.moduleTitle}</span>
                        </div>
                      </td>
